...
class Tab3(QWidget):
    def __init__(self, video_width=video_width, video_height=video_height, module_width=module_width, module_height=module_height):
        ...
        self.detect_broken_pixels_btn.clicked.connect(self.detect_broken_pixels_demo)
    ...
    def stretch_image(self, image, src_pts, width=32, height=16):
        ...
        stretched_image = cv2.warpPerspective(image, matrix, (int(width), int(height)))
    ...
    def detect_edges_dots(self):
        ...
        rec_x1, rec_y1, rec_x2, rec_y2 = split_coords[2:6]
        split_coords = list(map(float, split_coords[6:]))
        
        x1, y1, x2, y2, x3, y3, x4, y4 = list(map(int, split_coords))
        
        rectangle_points = [
            [(y1, x1), (abs(y4-y1)//2, abs(x4-x1)//2), (y4, x4)],
            [
                (abs(y2-y1)//2, abs(x1-x2)//2),
                (abs(abs(y2-y1)//2 + abs(y3+y4)//2)//2, abs(abs(x1-x2)//2 + abs(x3-x4)//2)//2),
                (abs(y3+y4)//2, abs(x3-x4)//2),
            ],
            [(y2, x2), (abs(y3-y2)//2, abs(x3+x2)//2), (y3, x3)],
            # [((x2-x1)//2, (y2-y1)//2), (abs(x4-x1)//1, (y4-y1)//2), (x4, y4)],
        ]
    ...
    def transform_to_rectangle(self):
        ...
        wwi, hhi, _ = self.image.shape
        module_loc = np.array(module_loc)

        w_temp = hhi // (self.video_height // self.module_height)
        h_temp = wwi // (self.video_width // self.module_width)
        
        self.out_image = np.zeros((h_temp*(self.video_height // self.module_height), w_temp*(self.video_width // self.module_width), 3), dtype=np.uint8)
        
        for i in range(h):
            for j in range(w):
                stretched_module = self.stretch_image(self.image, np.array(module_loc[j][i], dtype=np.float32), w_temp, h_temp)
                self.out_image[int(i*h_temp):int(i*h_temp+h_temp), int(j*w_temp):int(j*w_temp+w_temp)] = stretched_module
    ...
    def calculate_alpha_beta(self, orig, mod):
        A = np.vstack([orig, np.ones_like(orig)]).T  # shape (3, 2)
        solution, _, _, _ = np.linalg.lstsq(A, mod, rcond=None)
        s, beta = solution
        ...
        return alpha, beta
    def detect_broken_pixels_demo(self):
        image = self.out_image.copy()
        height, width, _ = image.shape
        
        module_h, module_w = height // (self.video_height // self.module_height), width // (self.video_width // self.module_width)

        sub_images = {}
        for i in range(self.video_height // self.module_height):
            for j in range(self.video_width // self.module_width):
                sub_images[(i, j)] = image[i*module_h:i*module_h+module_h, j*module_w:j*module_w+module_w]
        ...
        # Find keys with values out of range
        invalid_keys = [
            key for key, value in average_colors.items()
            if not np.all((lower_bound <= value) & (value <= upper_bound))
        ]
        
        valid_vals = [
            value for _, value in average_colors.items()
            if np.all((lower_bound <= value) & (value <= upper_bound))
        ]

        # print("ALLL", average_colors, invalid_keys, np.array(valid_vals).mean(axis=0).astype(int))
        # print("ALLLALA", average_colors, invalid_keys, valid_vals)
        
        for key in invalid_keys:
            alpha, beta = self.calculate_alpha_beta(np.array(valid_vals).mean(axis=0).astype(int), average_colors[key])
            print(f"FOUND WRONG MODEL at {key}, with {alpha} {beta}")
        
        for i, j in invalid_keys:
            image[i*module_h:i*module_h+module_h, j*module_w:j*module_w+module_w] = [0, 255, 0]
        ...
        with open('output.txt', "w") as file:
            for key in invalid_keys:
                alpha, beta = self.calculate_alpha_beta(np.array(valid_vals).mean(axis=0).astype(int), average_colors[key])
                x, y = key                
                file.write(f"({x}, {y}, {module_width}, {module_height}) {alpha} {beta}\n")        
