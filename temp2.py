...
color1_hsv = rgb_to_hsv([238, 147, 53])
...
color1_lower = np.array([30, 130, 220])
color1_upper = np.array([70, 180, 250])
...
def calculate_brightness_contrast(image, res_image, broken_module, white_modules):
    ...
    for i, white_module in enumerate(white_modules):
        ...
        curr_avg = np.mean(ref_img, axis=(0,1))
        ...
        brightness_diffs.append(np.mean(ref_img) - np.mean(image1))
        std_image1 = np.std(image1)
        std_ref_img = np.std(ref_img)
        contrast_factors.append(std_ref_img / std_image1 if std_image1 != 0 else 1)
    
    brightness_diff = np.mean(brightness_diffs)
    contrast_factor = np.mean(contrast_factors)
    ...
    # print('correct', np.mean(avg_arr, axis=0))
    ...
    if 288 <= x <= 320 and y == 96:
        print('do', x, y)
        # res_image[y:y+h, x:x+w] = [255, 255, 255]
        res_image[y:y+h, x:x+w] = np.clip(res_image[y:y+h, x:x+w] * contrast_factor + brightness_diff, 0, 255)
...
for broken_modules in blues_with_whites:
    ...
    calculate_brightness_contrast(s_image_orig, red_image, broken_module, white_modules)
